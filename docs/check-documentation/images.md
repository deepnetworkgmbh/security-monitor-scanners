...
`images.vulnerabilityScanFailed` | `error` | Returns `failed` when image has critical vulnerabilities and `warning` when has non-critical.
...
A software vulnerability is a glitch, flaw, or weakness present in the software or in an Operating System. Each OS and Application has numerous dependencies, each of which might have issues. During the scan process OS or Application packages can be analyzed if used package version has known issues or not.

Vulnerabilities scanner is not a part of `polaris` service, but an external `kube-scanner` application, which is a wrapper around [trivy](https://github.com/aquasecurity/trivy) OSS application.
